...
export async function POST(req: Request) {
  ...
  const window =
    range==='7d'  ? "r.requested_at >= NOW() - INTERVAL '7 days'"
  : range==='30d' ? "r.requested_at >= NOW() - INTERVAL '30 days'"
  : 'TRUE'

  try {
    const topRoutes = await pool.query(`
      SELECT lo.name AS origin, ld.name AS dest,
             COUNT(*)::int AS rides,
      ...
      WHERE ${window} AND r.status IN ('accepted','ongoing','completed')
    ...
    const riderSpend = await pool.query(`
      SELECT ri.rider_id, ri.name AS rider_name,
